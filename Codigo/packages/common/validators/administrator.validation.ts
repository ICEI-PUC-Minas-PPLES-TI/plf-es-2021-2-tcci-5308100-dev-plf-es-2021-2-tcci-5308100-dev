...
const validatorBase = {
  ...
  status: yup
    .mixed<AdministratorStatus>()
    .oneOf(Object.values(AdministratorStatus))
    .required(),
...
export const createAdministratorValidator: (
  body: any
) => Promise<ValidatorResult<CreateAdministratorDTO>> = async (body) => {
  ...
  const validator: yup.SchemaOf<CreateAdministratorDTO> = yup.object().shape({
    ...validatorBase,
  });
...
export const updateAdministratorValidator: (
  body: any
) => Promise<ValidatorResult<UpdateAdministratorDTO>> = async (body) => {
  ...
  const validator: yup.SchemaOf<UpdateAdministratorDTO> = yup.object().shape({
